...
int get(const Var& v){
    return (int)v.get(GRB_DoubleAttr_X);
}
...
LinePlanning::LineConcept _solve(const Instance& instance, const TreeDecomposition::TreeDecomposition& td, const Options &options) {
    ...
    public:
        NiceVisitor(const Instance *instance, GRBModel *model, const Options *options) : instance(instance), model(model), options(options), rTree(make_shared<ReconstructionTree::Leaf>()) {}
        ...
        void introduce(int v){
            ...
            //introduce
            for (auto u : vertices){
                auto pp = PP{{vertexRenaming[u], vertexRenaming[v]}};
